...
namespace PA4Draft
{
    public partial class MainForm : Form
    {
        ...
        private void TextureBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            // Image image = new Bitmap("891.jpg");
            // Image image = new Bitmap(p.fileName);
            // TextureBrush texture = new TextureBrush(image); 
            //  string file = p.fileName;
            // using (Brush brush = new TextureBrush(new Bitmap(file)) ;          
            //texture.WrapMode= System.Drawing.Drawing2D.WrapMode.TileFlipY;
            if (p.fileName != "")
            {
                Bitmap image = new Bitmap(p.fileName);
                TextureBrush texture = new TextureBrush(image);
                if (p.wrapMode == 1)
                {
                ...
                if (p.wrapMode == 2)
                {
                    texture.WrapMode = WrapMode.TileFlipX;
                }
                if (p.wrapMode == 3)
                {
                    texture.WrapMode = WrapMode.TileFlipY;
                }
                if (p.wrapMode == 4)
                {
        ...
        private void HatchBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            HatchBrush Brush = p.brush;
            shapes[shapeList.SelectedIndex].fillBrush = Brush;
            updateTileDesign();
            updateShapeList(shapeList.SelectedIndex);
        }
        private void LinearGradientBrush_Click(object sender, EventArgs e)
        {
            ...
            if (d != DialogResult.OK)
                return;
            shapes[shapeList.SelectedIndex].fillBrush = p.lBrush;
            updateTileDesign();
            updateShapeList(shapeList.SelectedIndex);
        }
